...
static const char* kStreamName = "playback";

Playback::Playback(const ErrorHandlerType& error_handler,
                   const BufferHandlerType& empty_buffer_handler)
    : error_handler_(error_handler),
      empty_buffer_handler_(empty_buffer_handler),
...
int Playback::Open(const Metadata& metadata) {
  ...
  int error;
  pulseaudio_handle_ = pa_simple_new(NULL, kClientName, PA_STREAM_PLAYBACK,
                                     NULL, kStreamName, &pulseaudio_spec, NULL,
                                     NULL, &error);
...
int Playback::Start() {
  ...
  thread_ = new thread(Playback::StaticThreadEntry, this);
  if (thread_ == nullptr)
    return -ECHILD;
...
int Playback::WriteBuffer(const Buffer& buffer) {
  Message message;
...
void Playback::StaticThreadEntry(Playback* playback) {
  playback->ThreadPlayback();
}

void Playback::ThreadPlayback() {
  ...
  while (keep_running) {
    ...
    if (keep_running) {
      while (!buffers.empty()) {
        ...
        result = pa_simple_write(pulseaudio_handle_, buffer.data, buffer.size,
                                 &error);
        if (result < 0) {
          error_handler_(-EIO);
          keep_running = false;
  ...
  result = pa_simple_drain(pulseaudio_handle_, &error);
  if (result < 0)
    error_handler_(-EIO);
}
